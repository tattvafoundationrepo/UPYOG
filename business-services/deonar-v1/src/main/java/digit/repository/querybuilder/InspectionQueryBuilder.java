...
public class InspectionQueryBuilder {
	...
	private static final String SEARCH_BASE_QUERY= """
			...
			a. arrivalId,b.stakeholdername, f.count \
			FROM \
			  \
			    public.eg_deonar_arrival a \
			LEFT JOIN \
			    public.eg_deonar_stakeholder b ON b.id = a.stakeholderid
			LEFT JOIN \
			    public.eg_deonar_animal_at_arrival f ON a.id = f.arrivalid
			
			""";

	public String getSearchBaseQuery(SecurityCheckCriteria securityCheckCriteria, List<Object> preparedStmtList)
	{
		...
		if (securityCheckCriteria.getArrivalUuid()!= null) {
			...
			preparedStmtList.add(securityCheckCriteria.getArrivalUuid());
		}
