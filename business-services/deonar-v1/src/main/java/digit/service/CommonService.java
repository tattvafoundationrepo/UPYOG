...
public class CommonService {
    ...
    public List<CommonDetails> getStakeholder(RequestInfo requestInfo,CommonSearchCriteria searchcriteria){
        // Fetch applications from database according to the given search criteria
        List<CommonDetails> common = commonRepository.getStakeholder(searchcriteria);
        // If no applications are found matching the given criteria, return an empty list
        if (CollectionUtils.isEmpty(common))
            return new ArrayList<>();
        return common;
