...
import { convertEpochFormateToDate, ConvertEpochToTimeInHours, getDayfromTimeStamp } from '../utils/index';
...
const WorkflowTimeline = ({ businessService, tenantId, applicationNo, timelineStatusPrefix = "WF_SERVICE_", statusAttribute = "status", ...props }) => {
    ...
    const getTimelineCaptions = (checkpoint, index) => {
        ...
        if (index === -1) {
            ...
            captionDetails.date = `${convertEpochFormateToDate(checkpoint.auditDetails.lastModifiedEpoch)} ${ConvertEpochToTimeInHours(
                checkpoint.auditDetails.lastModifiedEpoch
            )} ${getDayfromTimeStamp(checkpoint.auditDetails.lastModifiedEpoch)}`;
    ...
    let workflowDetails = Digit.Hooks.useWorkflowDetailsnew({
        tenantId,
        id: applicationNo,
