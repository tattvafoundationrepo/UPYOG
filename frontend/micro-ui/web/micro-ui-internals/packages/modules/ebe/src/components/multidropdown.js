...
import { ArrowDown, CheckSvg, RemoveableTag } from "@upyog/digit-ui-react-components";
  ...
  return (
    <div className="multi-select-dropdown-wrap" ref={dropdownRef}>
      ...
      <div className="selected-tags">
        {selected.length > 0 &&
          selected.map((option) => (
            <span className="tag-container" key={option[optionsKey]} style={{display: "inline-block"} }>
              <RemoveableTag text={t(option[optionsKey])} onClick={() => clearOption(option)} />
            </span>
          ))}
      </div>
    </div>
