...
import { animalStabling } from "../../../constants/dummyData";
...
const AnteMortermInspectionStatusPage = () => {
  ...
  console.log("data2", inspectionData);
  ...
  useEffect(() => {
    if (data2 && !isLoading) {
      ...
      setValue("sex", data2?.deonar?.sex[0]?.name || "");
    }
  }, [data2, isLoading, setValue]);
  ...
  const handleSave = (e) => {
    ...
    const InspectionType = "Ante Mortem Inspection";

    saveAnteMortemInspection.mutate(
      { ...data, arrivalId, InspectionType, tokenNumber, animalType, animalTypeId },
  ...
  const handleAdd = () => {
    ...
    const formattedData = {
      ...
      bodyTemp: formDataValues.bodyTemperature?.name || "",
  ...
  return (
    <React.Fragment>
      <div className="bmc-card-full">
        <MainFormHeader title={t("ANTE_MORTEM_INSPECTION")} />
...
export default AnteMortermInspectionStatusPage;