...
const Inbox = ({ parentRoute, businessService = "DEONAR", initialStates = {}, filterComponent, isInbox }) => {
    ...
    if (isMobile) {
      return (
        <MobileInbox
          ...
          linkPrefix={`/${window?.contextPath}/employee/deonar/details/`}
      ...
      return (
        <div>
          {isInbox && <Header>{t("BMC_DEONAR_DYNAMIC")}</Header>}
          <DesktopInbox
            businessService={businessService}
            data={data}
            isLoading={hookLoading}
