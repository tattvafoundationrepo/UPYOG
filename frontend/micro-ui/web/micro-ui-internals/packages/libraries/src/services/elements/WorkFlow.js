...
export const WorkflowService = {
  ...
  newgetDetailsByID: async ({ tenantId, id, moduleCode }) => {
    ...
    if (businessServiceResponse) {
      if (workflow && workflow.ProcessInstances.length > 0) {
        ...
        const getStateForUUID = (uuid) => businessServiceResponse?.find((state) => state.uuid === uuid);
        const currentState = businessServiceResponse?.filter((item) => item.state === null).map((item) => {
          let _nextActions = item.actions?.map?.((ac) => {
            let actionResultantState = getStateForUUID(ac.nextState);
            let assignees = actionResultantState?.actions?.reduce?.((acc, act) => {
              return [...acc, ...act.roles];
            }, []);
          ...
          //CHECK WHEN EDIT ACTION TO BE SHOWN
          return { ...item, nextActions: _nextActions, roles: item?.actions?.reduce((acc, el) => [...acc, ...el.roles], []) };
        })?.[0];;
        return currentState;
      }
    } else {
      console.log("Business Service Not Defined");
      return {};
    }
    return {};
  },
