...
const useWorkflowDetailsnew = ({ tenantId, id, moduleCode, role = ["CITIZEN","EMPLOYEE"], serviceData = {}, getStaleData, getTripData = false, config }) => {
    const queryClient = useQueryClient();
    
    const staleDataConfig = { staleTime: Infinity };
    const { isLoading, error, isError, data } = useQuery(
        ["workFlowDetailsWorks", tenantId, id, moduleCode, role, config],
        () => Digit.WorkflowService.newgetDetailsByID({ tenantId, id, moduleCode, role, getTripData }),
...
export default useWorkflowDetailsnew;